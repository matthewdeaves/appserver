...
- 004 — `appserver.sh init` requires an MFA-derived session. `AppserverAdmin` carries `DenyAllWithoutMFA` (Effect:Deny, NotAction:[GetUser, ChangePassword, MFA-management, sts:GetSessionToken, sts:GetCallerIdentity, ListAccessKeys], Resource:*, Condition:`aws:MultiFactorAuthPresent=false`). The shared admin user (rockport-admin) is enrolled with a TOTP device whose ARN goes in `terraform/.env` as `APPSERVER_ADMIN_MFA_SERIAL`. `cmd_init` calls `admin_mfa_session()` (in `scripts/lib/auth.sh`) at the top, which mints a 1-hour STS session via `sts:GetSessionToken` and caches it under the `appserver-admin-mfa` profile. `APPSERVER_AUTH_DISABLED=1` still bypasses (true bootstrap on a fresh account where the policy isn't deployed yet). Mirrors Rockport spec 018; the same MFA device serves both projects.
